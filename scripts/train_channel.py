...
for k, v in flat_config.items():
    ...
    elif k == "channel_model_kwargs//activation" and "activation" in "".join(sys.argv):
        parser.add_argument(
            f"--{k}",
            type=str,
            default=v.__name__ if v is not None else None,
            help=k,
        )
    else:
...
if isinstance(act_fn_name := flat_args["channel_model_kwargs//activation"], str):
    activation_fns = {
        "tanh": jax.nn.tanh,
    ...
    flat_args["channel_model_kwargs//activation"] = activation_fns[act_fn_name]
nested_inputs = nest_flattened_dict(flat_args)
...
"""
    ...
    channel_model = config.ChannelModel(
        key=rng_keys[1],
        **channel_kwargs,
    )
    ...
    # channel_data_train = data_train.Y.copy()
    # state_deriv = None
    if isinstance(channel_model, NODE):
        # state_deriv = jax.tree.map(
        #     lambda x: jnp.gradient(x, ts_train, axis=-1), channel_data_train
        # )
        # state_deriv["v"] *= init_params.get(".c", 1.0)
        
        # channel_model = channel_model.zscore_w_data(channel_data_train, state_deriv)

        _latents = channel_model.latent_states
        state_mean = {"v": jnp.array(-70.0), **{k: jnp.array(0.0) for k in _latents}}
        state_std = {"v": jnp.array(100.0), **{k: jnp.array(1.0) for k in _latents}}
        deriv_std = {"v": jnp.array(10.0), **{k: jnp.array(1.0) for k in _latents}}
        channel_model = channel_model.zscore(state_mean, state_std, None, deriv_std)

    init_model = hh.insert(channel_model)
    init_model = init_model.set(init_params)  # init params with true values
    ...
    for epoch, strat in enumerate(config.opt_strategy, start=1):
        ...
        model_labels = [label_params(diff_model, {"net": "nn"}, "ode")]
        optimizer = hybrid_optimizer(
            ode_optim=strat["ode_optim"],
            nn_optim=strat["nn_optim"],
            ode_vs_nn_labels=model_labels,
        )
...
```
hh = {hh.__repr__()}
channel_model = {config.ChannelModel.__name__}(key=jr.key(0), {kwargs_str})
init_model = hh.insert(channel_model)
loaded_model = eqx.tree_deserialise_leaves("{fout}.eqx", init_model)
